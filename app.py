...
@st.cache_resource
def load_summarizer():
    return pipeline("summarization", model="t5-small")  # lightweight, fast
...
def fetch_arxiv_abstracts(query, max_results=5):
    base_url = "http://export.arxiv.org/api/query?search_query=all:{}&start=0&max_results={}"
    feed_url = base_url.format(query, max_results)
    feed = feedparser.parse(feed_url)
...
if st.button("Summarize"):
    if user_text.strip():
        max_len = 50 if difficulty == "Beginner" else 100 if difficulty == "Intermediate" else 150
        raw_summary = summarizer(user_text, max_length=max_len, min_length=30, do_sample=False)[0]['summary_text']

        # Capitalize each sentence, remove extra spaces before periods, ensure final period
        sentences = sent_tokenize(raw_summary)
        cleaned_sentences = [s.strip().capitalize().rstrip(' .') + '.' for s in sentences]
